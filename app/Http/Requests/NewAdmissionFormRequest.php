...
class NewAdmissionFormRequest extends FormRequest
{
    ...
    public function rules()
    {
        ...
        foreach ($this->subject_preferences as $key => $pref) {
            foreach ($pref['selectedOpts'] as $option) {
                if ($option['subject_id'] > 0) {
                    ...
                    $adm_sub_pref->admission_id = $pref['admission_id'];
                    $adm_sub_pref->selected_ele_id = $pref['selected_ele_id'];
                    $adm_sub_pref->ele_group_id = $option['ele_group_id'];
                    $adm_sub_pref->sub_group_id = $option['sub_group_id'];
                    $adm_sub_pref->preference_no = intval($key) + 1;
        ...
        if ($this->last_exam_mcm == 'N') {
            foreach ($this->subject_preferences[0]['selectedOpts'] as $key => $option) {
                if ($option['subject_id'] > 0) {
                    $admsub = new AdmissionSubs();
                    $admsub->subject_id = $option['subject_id'];
                    $admsub->admission_id = $pref['admission_id'];
                    $admsub->ele_group_id = $option['ele_group_id'];
                    $admsub->sub_group_id = $option['sub_group_id'];
    ...
    public function save()
    {
        ...
        AdmissionSubPreference::createOrUpdateMany($this->adm_sub_prefs, ['admission_id' => $adm_form->id], $idarr3);
