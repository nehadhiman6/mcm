@if(auth()->check())
   <div class="page-sidebar custom-scroll" id="sidebar">
      ...
      <ul class="sidebar-menu metismenu">
         ...
         @can('edit-own-staff-details')
            @if(auth()->user()->staff)
               <li class = "{{ checkActive(['staff/*/edit']) }}">
                  <a href="{{url('staff') .'/'.auth()->user()->staff->id .'/edit'}}">
                     <i class="sidebar-item-icon ft-layers"></i><span>My Details</span>
                  </a>
               </li>
            @endif 
         @endcan
            ...
            <li>
               {{-- @if(Gate::check('ADMISSION-FORMS') || Gate::check('NEW-ADMISSION-FORMS') || Gate::check('SUBJECT-WISE-STRENGTH') || Gate::check('online-transactions') || Gate::check('ADMISSION-ENTRY') || Gate::check('NEW-ADMISSION-ENTRY')) --}}
                  <a href="javascript:;"><i class="sidebar-item-icon ft-layers"></i>
                     <span class="nav-label">College/Hostel Admission</span><i class="arrow la la-angle-right"></i>
                  </a>
               {{-- @endif --}}
               <ul class="nav-2-level">
                  ...
                  @if( hasAccessToMenuOption(['ADMISSION-ENTRY','NEW-ADMISSION-ENTRY']) )
                     <li class="{{ checkActive(['admission-entries','adm-entries','adm-entries/create']) }}">
         ...
         <li class="{{ checkActive(['students-toolbar','idcard-report','student-category-wise-report','consolidated-student-list','students','stdledger','stdstrength','std-subjects','sub-stdstrength','rmvstudents/create','rmvstudents']) }}">
         ...
         <li>
            ...
            <ul class="nav-2-level">
               ...
               @endif
               @if( hasAccessToMenuOption(['BILL-CANCELLATION','ONLINE-TRANSACTION-STATUS','FEE-INSTALLMENTS','HOSTEL-FEE-INSTALLMENTS','CENTERALIZED-FEE-INSTALLMENTS','STUDENT-SUBJECT-CHARGES']) )
                  <li class = "{{ checkActive(['bill-cancellation','bill/cancel','checktrans','fee-insts/create','host-fee-insts/create','cent-fee-insts/create','stdsubcharges']) }}">
               ...
               @if( Gate::check('prospectus-fees') || Gate::check('idcard-report') || Gate::check('daybook') || Gate::check('daybook2') || Gate::check('dbsummary') || Gate::check('feeheadwise-coll') || Gate::check('subheadwise-coll') || Gate::check('student-feedback-report') )
                  <li class = "{{ checkActive(['daybook-report','prospectus-fees','admission-form/payments','daybook','daybook2','dbsummary','feeheadwise-coll','subheadwise-coll','student-feedback-report']) }}">
         ...
         {{-- @if( Gate::check('staff-list') || Gate::check('designation-list') || Gate::check('department-list') || Gate::check('faculty-list') )
            <li class="{{ checkActive(['staff-toolbar','staff','designation','department','faculty','staff-report-toolbar']) }}">
               ...
               @endif
                  ...
                  @if( hasAccessToMenuOption(['alumni-sms-mail','send-regsms-alumni']) ) 
                     <li class = "{{ checkActive(['send-sms-alumni','sms-report']) }}">
                        <a href="{{url('sms-report')}}">Send SMS/Email</a>
                     </li>
                  @endcan
            </ul>
         </li>
            ...
            <ul class="nav-2-level">
               ...
               @if( hasAccessToMenuOption(['inv-vendor','add-inv-vendor']) )
                  <li class = "{{ checkActive(['vendors-toolbar','vendors','vendors/create','vendors/*/edit']) }}">
                     <a href="{{url('vendors-toolbar')}}">Vendor</a>
                  </li>
               @endif
               ...
               @if( hasAccessToMenuOption(['inv-purchase-return','add-inv-purchase-return']) )
                  <li class = "{{ checkActive(['purchase-returns-toolbar','purchase-returns','purchase-returns/create','purchase-returns/*/edit']) }}">
                     <a href="{{url('purchase-returns-toolbar')}}">Purchase Returns</a>
                  </li>
               @endif
               @if( hasAccessToMenuOption(['inv-return','add-inv-return']) )
                  <li class = "{{ checkActive(['returns-toolbar','returns','returns/create','returns/*/edit']) }}">
                     <a href="{{url('returns-toolbar')}}">Returns</a>
                  </li>
               @endif
               @if( hasAccessToMenuOption(['inv-damage','add-inv-damage']) )
                  <li class = "{{ checkActive(['damages-toolbar','damages','damages/create','damages/*/edit']) }}">
                     <a href="{{url('damages-toolbar')}}">Damages</a>
                  </li>
               @endif
               @if( hasAccessToMenuOption(['opening-stocks','add-opening-stocks']) )
                  <li class = "{{ checkActive(['opening-stocks-toolbar','opening-stocks','opening-stocks/create']) }}">
                     <a href="{{url('opening-stocks-toolbar')}}">Opening Stock</a>
                  </li>
               @endif
         ...
         @if(Gate::check('MESSAGES') || Gate::check('MESSAGES-STAFF') )
            <li class="{{ checkActive(['messages','staffmsg','messages-toolbar']) }}">
               <a href="{{url('messages-toolbar')}}"><i class="sidebar-item-icon ft-edit"></i><span class="nav-label">Send SMS</span></a>
            </li>
         @endif
