...
<script>
  $(document).on('click','.subject-link',function(e) {
    e.preventDefault();
    $('#subject-box').html('');
    $('#subject-box').append('<form id="subject-form" action=\'{{ url("subwise-stddetails") }}\' method="GET" target="_blank">');
  ...
  //      "serverSide": true,
        ...
        columnDefs: [
          ...
          { title: '', targets: 3, data: 'subject',
             "render": function( data, type, row, meta) {
              return "<button class='btn btn-primary btn-xs subject-link' data-subject-name='"+row.subject+"' data-subject-id='"+row.subject_id+"' >Details</button>";
          }},
...
//              console.log(colNo);
  ...
  methods: {
    getData: function() {
        ...
        this.$http.get("{{ url('sub-stdstrength') }}", {params: data})
