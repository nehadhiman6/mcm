...
<div class="box box-default box-solid" id='app'>
  ...
  <div class="box-body">
    {!! Form::open(['method' => 'GET',  'action' => ['Reports\AdmStrengthController@subStdStrength'], 'class' => 'form-horizontal']) !!}
...
<script>
  $(document).on('click','.subject-link',function(e) {
    e.preventDefault();
    $('#subject-box').html('');
    $('#subject-box').append('<form id="subject-form" action=\'{{ url("subwise-admdetails") }}\' method="GET" target="_blank">');
  ...
  //      "serverSide": true,
        ...
        columnDefs: [
          ...
          { title: '', targets: 3, data: 'subject',
              "render": function( data, type, row, meta) {
                var str = '';
                if(self.permissions['SUBJECT-WISE-STRENGTH-DETAIL']){
                    str += "<button class='btn btn-primary btn-xs subject-link' data-subject-name='"+row.subject+"' data-subject-id='"+row.subject_id+"' >Details</button>";
                }
                  return str;
              }
...
//              console.log(colNo);
  ...
  methods: {
    getData: function() {
        ...
        this.$http.get("{{ url('sub-admstrength') }}", {params: data})
