...
class AddExamFeeForBA1 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'examfees:addforba1';
     ...
     * @var string
     */
    protected $description = 'Debits BA1 students for Exam Fees!';
    ...
    public function handle()
    {
        ...
        $students = Student::orderBy('students.id')
            ...
            ->whereIn('students.id', function ($q) use ($course_id) {
                $q->from('course_subject')
                    ->join('student_subs', 'student_subs.subject_id', '=', 'course_subject.subject_id')
                    ->where('course_subject.course_id', $course_id)
                    ->where('practical', 'Y')
                    // ->whereIn('fee_bill_dets.subhead_id', $pract_ids)
                    ->select('student_subs.student_id');
            })
            ->whereNotIn('students.id', function ($q) use ($exam_sh_id) {
                $q->from('fee_bills')
                    ...
                    ->where('bill_date', '>', Carbon::createFromFormat('d-m-Y', '31-12-2018'))
        ...
        while ($stds->count() > 0) {
            foreach ($stds as $std) {
                ...
                $feebilldet->fill(['feehead_id' => $subhead->feehead_id, 'subhead_id' => $subhead->id, 'amount' => $exam_fee,
                    'concession' => 0, 'index_no' => 1]);
                $feebill = new FeeBill();
                $feebill->fill(['course_id' => $std->course_id, 'std_type_id' => $std->std_type_id,
