      ...
      <tbody>
        ...
        <tr>
          <td>
            ...
            @elseif($pay->trn_type == 'alumni_life_fee')<span>Alumni Life Membership Fee</span>
            @elseif($pay->trn_type == 'alumni_meet_fee')<span>Alumni Meet Fee</span>
            @endif
          </td>
          <td>{{ $pay->amt }}</td>
          <td>{{ $pay->trntime }}</td>
          <td>
            @if($pay->ourstatus == 'OK')
              ...
              @else
                Successful 
                {{-- <a href="{{ url('stdreceipts/' . $pay->fee_rcpt_id . '/printreceipt') }}" target="_blank" class="btn btn-sm btn-primary">Print</a> --}}
              @endif
            @else 
              Failed
            @endif
